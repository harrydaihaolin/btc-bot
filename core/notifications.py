...
class NotificationManager:
    ...
    def send_email_notification(self, all_courts: Dict[str, List[Dict]]) -> bool:
        ...
        try:
            ...
            if notification_id in self.sent_notifications:
                self.logger.info(f"Email notification already sent for this court combination: {notification_id}")
                return True
            
            total_courts = sum(len(courts) for courts in all_courts.values())
            self.logger.info(f"Sending notification for {total_courts} available courts across all dates")
    ...
    def send_sms_notification(self, all_courts: Dict[str, List[Dict]]) -> bool:
        ...
        try:
            ...
            notification_id = self.create_notification_id(all_courts_flat, "multi_date")
            if notification_id in self.sent_notifications:
                self.logger.info(f"SMS notification already sent for this court combination: {notification_id}")
                return True
            
            total_courts = sum(len(courts) for courts in all_courts.values())
            self.logger.info(f"Sending SMS notification for {total_courts} available courts across all dates")
    ...
    def _create_email_message(self, all_courts: Dict[str, List[Dict]], total_courts: int) -> str:
...
BTC Booking Bot 🤖
    ...
    def _create_sms_message(self, all_courts: Dict[str, List[Dict]], total_courts: int) -> str:
    ...
    def _send_email(self, message: str, total_courts: int) -> bool:
        ...
        try:
            ...
            smtp_port = 587
            sender_email = self.credentials.get('gmail_app_email') or self.credentials.get('notification_email')
            ...
            text = msg.as_string()
            server.sendmail(sender_email, self.credentials.get('notification_email'), text)
            server.quit()
        ...
        try:
            ...
            for carrier, sms_email in sms_gateways.items():
                try:
                    ...
                    server.starttls()
                    app_password = self.credentials.get('gmail_app_password')
                    server.login(self.credentials.get('gmail_app_email') or self.credentials.get('notification_email'), app_password)
                    text = msg.as_string()
                    server.sendmail(self.credentials.get('notification_email'), sms_email, text)
                    server.quit()
