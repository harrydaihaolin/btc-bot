...
class UBCConfig:
    ...
    def get_monitoring_config(self) -> Dict[str, int]:
        ...
        return {
            "monitoring_interval": int(
                os.getenv("UBC_MONITORING_INTERVAL", "5")
            ),  # minutes
    ...
    def get_booking_preferences(self) -> Dict[str, Any]:
        ...
        return {
            "preferred_courts": (
                os.getenv("UBC_PREFERRED_COURTS", "").split(",")
                if os.getenv("UBC_PREFERRED_COURTS")
                else []
            ),
            "preferred_times": (
                os.getenv("UBC_PREFERRED_TIMES", "").split(",")
                if os.getenv("UBC_PREFERRED_TIMES")
                else []
            ),
            "preferred_duration": os.getenv("UBC_PREFERRED_DURATION", "1"),  # hours
            "max_price": float(os.getenv("UBC_MAX_PRICE", "50.0")),
            "prime_hours_only": os.getenv("UBC_PRIME_HOURS_ONLY", "false").lower()
            == "true",
        }
    ...
    def validate_credentials(self) -> bool:
        ...
        try:
            ...
            if notif_config["sms_phone"] and not (
                notif_config["twilio_sid"]
                and notif_config["twilio_token"]
                and notif_config["twilio_phone"]
            ):
