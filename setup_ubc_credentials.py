...
def setup_ubc_credentials():
    ...
    while True:
        email = input("Enter notification email (optional, press Enter to skip): ").strip()
        if not email:
    ...
    if email:
        while True:
            gmail_password = getpass.getpass("Enter Gmail app password (optional, press Enter to skip): ").strip()
    ...
    print()
    sms_phone = input("Enter SMS phone number (optional, press Enter to skip): ").strip()
    if not sms_phone:
