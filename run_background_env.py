...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("btc_background_env.log"), logging.StreamHandler()],
)
...
class BackgroundMonitor:
    ...
    def run_initial_scan(self):
        ...
        try:
            ...
            if total_courts > 0:
                self.logger.info(
                    f"Found {total_courts} available courts in initial scan!"
                )
    ...
    def run_continuous_monitoring(self):
        ...
        self.logger.info(
            f"Starting continuous background monitoring (every {monitoring_interval} minutes)"
        )
        ...
        while self.running and (attempt < max_attempts or max_attempts == 0):
            try:
                ...
                if self.running and (attempt < max_attempts or max_attempts == 0):
                    self.logger.info(
                        f"Waiting {monitoring_interval} minutes before next scan..."
                    )
