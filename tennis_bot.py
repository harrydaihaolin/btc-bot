...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("tennis_bot.log"), logging.StreamHandler()],
)
...
class TennisCourtBot:
    ...
    def __init__(self, facility: str = "BTC"):
        ...
        if self.facility == "BTC":
            ...
            raise ValueError(
                f"Unsupported facility: {facility}. Supported facilities: BTC, UBC"
            )
    ...
    def setup_credentials(self) -> Dict[str, str]:
        ...
        print("=" * 50)
        print(
            f"Setting up your credentials for the {self.facility} tennis booking bot..."
        )
        print()
    ...
    def run_continuous_monitoring(
        self, interval_minutes: int = 5, max_attempts: int = 0
    ):
        """Run continuous monitoring"""
        logger.info(
            f"Starting continuous {self.facility} monitoring (every {interval_minutes} minutes)"
        )
        ...
        while True:
            try:
                ...
                if max_attempts > 0 and attempt_count >= max_attempts:
                    logger.info(
                        f"Reached maximum attempts ({max_attempts}), stopping monitoring"
                    )
                    break
    ...
    def run_timeslot_monitoring(
        self, interval_seconds: int = 30, max_attempts: int = 0
    ):
        """Run timeslot monitoring (faster scanning)"""
        logger.info(
            f"Starting timeslot {self.facility} monitoring (every {interval_seconds} seconds)"
        )
    ...
    if available_courts:
        ...
        for date, courts in available_courts.items():
            ...
            for i, court in enumerate(courts, 1):
                print(
                    f"   {i}. {court.get('court_name', 'Unknown')} at {court.get('time', 'Unknown')}"
                )
