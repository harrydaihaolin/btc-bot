...
class BaseConfig(ABC):
    ...
    def get_monitoring_config(self) -> Dict[str, int]:
        ...
        return {
            'monitoring_interval': int(os.getenv(f'{prefix}_MONITORING_INTERVAL', '5')),  # minutes
            'max_attempts': int(os.getenv(f'{prefix}_MAX_ATTEMPTS', '0')),  # 0 = unlimited
    ...
    def get_logging_config(self) -> Dict[str, str]:
        ...
        prefix = self.facility_name.upper()
        return {
            'log_file': os.getenv(f'{prefix}_LOG_FILE', f'{self.facility_name.lower()}_monitoring.log'),
    ...
    def get_booking_preferences(self) -> Dict[str, Any]:
        ...
        prefix = self.facility_name.upper()
        return {
            'preferred_courts': os.getenv(f'{prefix}_PREFERRED_COURTS', '').split(',') if os.getenv(f'{prefix}_PREFERRED_COURTS') else [],
            'preferred_times': os.getenv(f'{prefix}_PREFERRED_TIMES', '').split(',') if os.getenv(f'{prefix}_PREFERRED_TIMES') else [],
            'preferred_duration': os.getenv(f'{prefix}_PREFERRED_DURATION', '1'),  # hours
            'max_price': float(os.getenv(f'{prefix}_MAX_PRICE', '50.0')),
            'prime_hours_only': os.getenv(f'{prefix}_PRIME_HOURS_ONLY', 'false').lower() == 'true'
        }
