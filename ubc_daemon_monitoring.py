...
class UBCDaemonMonitor:
    ...
    def validate_configuration(self) -> bool:
        ...
        try:
            ...
            if not notif_config['email'] and not notif_config['sms_phone']:
                self.logger.error("❌ No notification method configured (email or SMS required)")
                return False
    ...
    def run_initial_scan(self) -> Dict[str, List[Dict]]:
        ...
        try:
            ...
            if new_courts:
                ...
                self.logger.info(f"Found {total_courts} available UBC courts in initial scan!")
    ...
    def run_monitoring_cycle(self) -> None:
        ...
        try:
            ...
            # Check max attempts
            if monitoring_config['max_attempts'] > 0 and self.attempt_count > monitoring_config['max_attempts']:
                self.logger.info(f"Reached max attempts ({monitoring_config['max_attempts']}), stopping monitoring")
                self.running = False
                return
            ...
            if new_courts:
                ...
                self.logger.info(f"🎾 NEW UBC COURTS DETECTED! {total_new} new slots found!")
            ...
            self.logger.info(f"Waiting {interval_minutes} minutes before next UBC scan...")
    ...
    def start_monitoring(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Starting continuous UBC background monitoring (every {monitoring_config['monitoring_interval']} minutes)")
            ...
            while self.running:
                try:
                    ...
                    self.logger.info("Received keyboard interrupt, stopping UBC monitoring...")
                    break
                except Exception as e:
