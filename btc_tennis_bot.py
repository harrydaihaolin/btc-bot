...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("btc_booking.log"), logging.StreamHandler()],
)
...
def setup_credentials():
    ...
    try:
        ...
        if not credentials["notification_email"]:
            credentials["notification_email"] = input(
                "Enter notification email (default: same as login): "
            ).strip()
            if not credentials["notification_email"]:
        ...
        if not credentials["phone_number"]:
            credentials["phone_number"] = input(
                "Enter your phone number (e.g., 6479370971): "
            ).strip()

        if not credentials["gmail_app_email"]:
            credentials["gmail_app_email"] = input(
                "Enter Gmail address for SMTP (default: same as notification email): "
            ).strip()
            if not credentials["gmail_app_email"]:
        ...
        if not credentials["gmail_app_password"]:
            ...
            print()
            credentials["gmail_app_password"] = getpass.getpass(
...
def run_single_scan(monitor, notification_manager):
    ...
    try:
        ...
        try:
            screenshot_path = (
                f"btc_booking_page_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
            )
...
def run_continuous_monitoring(
    monitor, notification_manager, interval_minutes=5, max_attempts=10
):
...
def main():
    ...
    try:
        ...
        try:
            ...
            if choice == "1":
                interval = int(
                    input("Enter monitoring interval in minutes (default 5): ") or "5"
                )
                max_attempts = int(
                    input("Enter maximum number of scans (default 10): ") or "10"
                )
                run_continuous_monitoring(
                    monitor, notification_manager, interval, max_attempts
                )
            elif choice == "2":
                interval = int(
                    input("Enter monitoring interval in seconds (default 30): ") or "30"
                )
                max_attempts = int(
                    input("Enter maximum number of checks (default 120): ") or "120"
                )
                # Convert seconds to minutes for the monitoring function
                run_continuous_monitoring(
                    monitor, notification_manager, interval / 60, max_attempts
                )
            else:
                print("Exiting...")
        except EOFError:
