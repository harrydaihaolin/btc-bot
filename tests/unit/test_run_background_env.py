...
class TestBackgroundMonitor:
    ...
    def test_check_environment_variables_success(self):
        ...
        with patch.object(
            self.monitor.config_manager, "validate_credentials", return_value=True
        ):
    ...
    def test_check_environment_variables_failure(self):
        ...
        with patch.object(
            self.monitor.config_manager, "validate_credentials", return_value=False
        ):
            result = self.monitor.check_environment_variables()
    ...
    def test_initialize_components_success(
        ...
        with patch.object(
            self.monitor.config_manager,
            "get_credentials",
            return_value={"username": "test@example.com"},
        ):
            with patch.object(
                self.monitor.config_manager,
                "get_bot_config",
                ...
                with patch.object(
                    ...
                    return_value={"monitoring_interval": 5},
                ):
                    with patch.object(
                        self.monitor.config_manager,
    ...
    def test_run_continuous_monitoring_success(self):
        ...
        with patch.object(
            self.monitor.config_manager,
            "get_monitoring_config",
            return_value={"monitoring_interval": 1, "max_attempts": 1},
        ):
            mock_courts = {
                "2025-10-26": [
                    {
            ...
            with patch(
                "run_background_env.time.sleep", side_effect=KeyboardInterrupt()
            ):
