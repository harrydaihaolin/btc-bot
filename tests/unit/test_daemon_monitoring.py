...
class TestDaemonMonitor(unittest.TestCase):
    ...
    def test_initialize_components_success(self):
        ...
        with patch.object(
            self.daemon_monitor.config_manager, "get_credentials"
        ) as mock_get_credentials, patch.object(
            self.daemon_monitor.config_manager, "get_bot_config"
        ) as mock_get_bot_config, patch.object(
            self.daemon_monitor.config_manager, "get_monitoring_config"
        ) as mock_get_monitoring_config, patch.object(
            self.daemon_monitor.config_manager, "validate_credentials"
            ...
            with patch("daemon_monitoring.CourtMonitor") as mock_court_monitor, patch(
                "daemon_monitoring.NotificationManager"
    ...
    def test_run_monitoring_cycle_success(self):
        ...
        self.daemon_monitor.notification_manager.send_email_notification.return_value = (
            True
        )
        self.daemon_monitor.notification_manager.send_sms_notification.return_value = (
            True
        )
    ...
    def test_run_daemon_success(self):
        ...
        with patch.object(
            ...
            # Mock other methods
            with patch.object(
                self.daemon_monitor, "initialize_components"
            ) as mock_initialize_components, patch.object(
                self.daemon_monitor, "run_monitoring_cycle"
            ) as mock_run_monitoring_cycle, patch(
                "daemon_monitoring.signal.signal"
            ), patch(
                "daemon_monitoring.time.sleep"
            ):
    ...
    def test_run_daemon_exception(self):
        ...
        with patch.object(
            ...
            with patch.object(
                self.daemon_monitor, "initialize_components"
            ) as mock_initialize_components, patch(
                "daemon_monitoring.signal.signal"
            ), patch(
                "daemon_monitoring.time.sleep"
            ):

                mock_initialize_components.side_effect = Exception(
    ...
    def test_run_daemon_unlimited_attempts(self):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                self.daemon_monitor, "running", False
